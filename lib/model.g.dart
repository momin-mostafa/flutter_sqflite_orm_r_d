...
class TableTodo extends SqfEntityTableBase {
  TableTodo() {
    ...
    // declare fields
    fields = [
      SqfEntityFieldBase('title', DbType.text, defaultValue: ''),
      SqfEntityFieldBase('is_completed', DbType.bool, defaultValue: false),
    ];
    super.init();
  }
...
class MyDatabaseModel extends SqfEntityModelProvider {
  MyDatabaseModel() {
...
class Todo extends TableBase {
  Todo({this.id, this.title, this.is_completed}) {
    _setDefaultValues();
    softDeleteActivated = false;
  }
  Todo.withFields(this.title, this.is_completed) {
    _setDefaultValues();
  }
  Todo.withId(this.id, this.title, this.is_completed) {
    _setDefaultValues();
  }
  // fromMap v2.0
  Todo.fromMap(Map<String, dynamic> o, {bool setDefaultValues = true}) {
    ...
    if (o['is_completed'] != null) {
      is_completed = o['is_completed'].toString() == '1' ||
          o['is_completed'].toString() == 'true';
    }
  }
  // FIELDS (Todo)
  int? id;
  ...
  Map<String, dynamic> toMap(
    ...
    if (is_completed != null) {
      map['is_completed'] = forQuery ? (is_completed! ? 1 : 0) : is_completed;
    } else if (is_completed != null || !forView) {
  ...
  static Future<List<dynamic>> saveAll(List<Todo> todos,
    ...
    final isStartedBatch = await MyDatabaseModel().batchStart();
    for (final obj in todos) {
      await obj.save(ignoreBatch: false);
    }
    if (!isStartedBatch) {
      result = await MyDatabaseModel().batchCommit(
          exclusive: exclusive,
          noResult: noResult,
  ...
  Future<int?> upsert({bool ignoreBatch = true}) async {
    try {
      final result = await _mnTodo.rawInsert(
          'INSERT OR REPLACE INTO todos (id, title, is_completed)  VALUES (?,?,?)',
...
class TodoFilterBuilder extends ConjunctionBase {
  ...
  TodoField? _is_completed;
  TodoField get is_completed {
    return _is_completed =
        _setField(_is_completed, 'is_completed', DbType.bool);
  }
...
class TodoFields {
  ...
  static TableField? _fIs_completed;
  static TableField get is_completed {
    return _fIs_completed = _fIs_completed ??
        SqlSyntax.setField(_fIs_completed, 'is_completed', DbType.bool);
  }
}
...
class IdentitySequence {
  ...
  Future<int> nextVal([VoidCallback Function(int o)? nextval]) async {
    final val = await MyDatabaseModelSequenceManager()
...
class MyDatabaseModelSequenceManager extends SqfEntityProvider {
  MyDatabaseModelSequenceManager() : super(MyDatabaseModel());
}
// END OF ENTITIES
